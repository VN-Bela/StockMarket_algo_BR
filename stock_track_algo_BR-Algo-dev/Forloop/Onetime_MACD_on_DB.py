...
def MACD_plot(data, fig, symbol, short_window=13, large_window=34):
...
while True:
    ...
    for stock_symbol in stocks:
        ...
        msg = plot_ema = MACD_plot(data=final_df, fig=fig, short_window=34, large_window=81, symbol= stock_symbol.upper())
...
#         # Find the list of index numbers which have the value in the date column as the date_to_fetch
#         index_list = sql_df.index[sql_df['Date'] == date_to_fetch_from].tolist()
#         for idx in range(index_list[-1] + 1, sql_df.shape[0]):
#             sql_df.drop(idx, inplace=True)
...
#                 print(f"ReadTimeout occurred for {stock_symbol}: {e}")
#                 retry_count += 1
#                 if retry_count >= max_retries:
#                     print(f"Failed to fetch data for {stock_symbol} after {max_retries} retries.")
#                     break  # Exit the retry loop after max retries
#             except AttributeError as e:
#                 print(f"AttributeError occurred for {stock_symbol}: {e}")
#                 retry_count += 1
#                 if retry_count >= max_retries:
#                     print(f"Failed to fetch data for {stock_symbol} after {max_retries} retries.")
#                     break  # Exit the retry loop after max retries
...
#             continue  # Skip to the next stock symbol if data fetch failed
...
#         msg = MACD_plot(data=final_df, fig=fig, short_window=13, large_window=34, symbol=stock_symbol.upper())
#         print(msg)
...
#     print(combine_msg)
#     time.sleep(1)  # Adding a sleep to avoid tight loop, can be adjusted as needed